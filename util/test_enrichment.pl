...
# Opening commands
my $usage = "Usage: perl $0 -a <d2go_out.processed> -b <subset.list>\n
...
my $all_GO_terms = tabfile::save_columns_to_one_hash($opt_a, 0, 4);
...
&print_GO_terms_for_all_D2GO($all_GO_terms, $file2_compare);
...
sub print_GO_terms_for_all_D2GO {
	my ($genes_to_go, $outfile) = @_;

	warn "print_GO_terms_for_all_D2GO: $outfile\n";
	open my $ofh, '>', $outfile or die "Error: cannot open $outfile : $!";
	my $gene_count = 0;
	foreach my $gene(sort keys %{$genes_to_go}) {
		$gene_count++;
